...
/**
 * The product explainer: what CampusFix does, how a report moves, what the
 * dashboard looks like, and who sees what. Kept as its own page so it can be
 * shown to a college or linked from the marketing site, separate from the
 * landing experience.
 */
export function Product() {
  return (
    <>
      <section className="shell-wide" style={{ paddingBlock: '72px 0' }}>
        <span className="overline">The product</span>

        <h1 className="display" style={{ fontSize: 'clamp(36px, 6vw, 64px)', margin: '18px 0 20px', maxWidth: '16ch' }}>
          Report it. Track it. See it <em style={{ color: 'var(--clay)' }}>fixed</em>.
        </h1>
        ...
        <nav className="contents-dark" aria-label="On this page" style={{ marginTop: 36, maxWidth: 520 }}>
          {CONTENTS.map((c) => (
        ...
        <div className="row wrap" style={{ gap: 12, marginTop: 36 }}>
          <Link to="/signup" className="btn btn-lg">Create an account</Link>
          <Link to="/login" className="btn btn-ghost btn-lg">See the demo</Link>
        </div>
      </section>
